...
from accuracy_eval.encryption.paillier import PaillierPublicKey, PaillierPrivateKey
from accuracy_eval.encryption import aciq
...
def clif_with_tf_clip_by_norm(grads, thresholds):
    return [tf.clip_by_global_norm(x, y) for x, y in zip(grads, thresholds)]
...
def calculate_clip_tensorflow_new(grads):
    # global_norm = sqrt(sum([numpy.linalg.norm(t) ** 2 for t in grads]))
    res = []
    for idx in range(len(grads)):
        clip_norm = scipy.linalg.norm(grads[idx], ord=None, axis=None, keepdims=False, check_finite=True)
        res.append(clip_norm)
    # res = tf.clip_by_norm(grads, clip_norm, axes=None, name=None)
    return res
