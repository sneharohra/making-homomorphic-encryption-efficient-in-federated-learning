...
from accuracy_eval.encryption import encryption, paillier
...
def build_datasets(num_clients):
    ...
    # Create a new dataset with some noisy data points
    batch_size = 1000
    indices_to_modify = [x for x in range(1, 1000, 8)]  # indices of data points to modify
    images = train_images[:batch_size]
    noisy_image = tf.tensor_scatter_nd_update(images, tf.expand_dims(indices_to_modify, axis=1),
                                               add_gaussian_noise(images[indices_to_modify]))

    for k in range(1, len(train_images)):
        j=0
        if k % 8 == 0:
            train_images[k] = noisy_image[j]
            j += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', type=str, default="plain",
                        choices=["plain", "batch", "only_quan", "aciq_quan", "tf_clip"])
    ...
    for epoch in range(num_epochs):
        ...
        epoch_accuracy = keras.metrics.Accuracy()
        ...
        for data_clients in zip(*train_dataset_clients):
            ...
            # federated_lr_batch.py
                ...
                for i in range(len(enc_grads)):
                    ...
                    plain_g = encryption.decrypt_matrix_batch(privatekey, enc_grads[i],
                                                              enc_grads_shape_batch_clients[0][i])
                    grads.append(plain_g)

            # federated_lr_only_quan.py
            elif args.experiment == "only_quan":
                for idx in range(len(grads_batch_clients)):
                    grads_batch_clients[idx] = [x for x in grads_batch_clients[idx]]
                ...
                for layer_idx in range(len(grads_batch_clients[0])):
                    ...
                    min_values.append([np.min([item[layer_idx] for item in grads_batch_clients])])
                grads_max_min = np.concatenate([np.array(max_values), np.array(min_values)], axis=1)
                clipping_thresholds = encryption.calculate_clip_tensorflow_new(grads_max_min)
    ...
    print("Saved model to disk")
    print("Full Time taken = ", time.time() - starttime)
