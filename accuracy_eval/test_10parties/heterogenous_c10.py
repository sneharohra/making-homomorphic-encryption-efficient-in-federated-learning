...
def build_datasets(num_clients):
    ...
    for k in range(num_clients):
        if k % 3 == 0:
            gray_images = np.dot(x_train_clients[k], [0.2989, 0.5870, 0.1140])  # convert to grayscale using the formula
            x_train_clients[k] = np.stack([gray_images] * 3, axis=-1)
            x_train_clients[k] = x_train_clients[k].astype('float32')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', type=str, default='plain_alex',
                        choices=["plain_alex", "plain_alex_en", "en_alex_batch", "aciq_quan", "tf_clip"])
    ...
    print("TIME TAKEN = ", time.time() - start_time_main)
