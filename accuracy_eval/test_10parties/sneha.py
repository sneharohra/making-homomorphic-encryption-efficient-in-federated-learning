...
from accuracy_eval import augmentation
from accuracy_eval.encryption import paillier, encryption
...
model.add(keras.layers.MaxPooling2D(pool_size=(2, 2), input_shape=x_train.shape[1:]))
...
# model.add(tf.keras.layers.Reshape((-1, 32, 32, 3)))
model.summary()
print("in", model.input_shape)
print("op", model.output_shape)
s = time.time()


# x = keras.Input(shape=(None, 224, 224, 3))
# y = keras.model.Dense(16, activation='softmax')(x)
# model = Model(x)
# model = keras.Sequential()
# input_shape = (18432,)
# input_layer = Input(shape=input_shape)
# model2 = keras.Sequential()
# model2 = Model(inputs=model.input, outputs=model.layers[-10].output)
# # model = model2


# model.add(keras.layers.MaxPooling2D(pool_size=(2, 2), input_shape=x_train.shape[1:]))
# model.add(keras.model.Dense(32, input_shape=(None, 224, 224, 3)))
...
# out2 = model.dense(x)
# tf.stop_gradient(keras.layers.Dense(512))
# tf.stop_gradient(keras.layers.Flatten())
# tf.stop_gradient(keras.layers.Dropout(0.5))
        ...
        for data_clients in zip(*train_dataset_clients):
            ...
            # federated_lr_en_alex_batch.py part
                ...
                clipping_thresholds = theta * (
                        ...
                        - (np.sum(grads_batch_clients_mean * layers_size, 0) / (
                        layers_size * args.num_clients)) ** 2) ** 0.5
